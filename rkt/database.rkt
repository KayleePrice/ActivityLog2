...
;; Geoids are stored as a signed 64 bit number in the database.  This is to
;; work around the problem where SQLite will only store signed 64 bit values
;; and will convert large unsigned 64 bit values into doubles
(define geoid-offset (arithmetic-shift 1 63))
        ...
        (fields
         `(timestamp position-lat position-long altitude distance cadence
                     ...
                     ,(lambda (tp)
                        (let ((lat (dict-ref tp 'position-lat #f))
                              (lon (dict-ref tp 'position-long #f)))
                          (and lat lon (- (lat-lng->geoid lat lon) geoid-offset)))))))
